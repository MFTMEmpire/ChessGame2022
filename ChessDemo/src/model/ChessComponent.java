...
//    private static final Dimension CHESSGRID_SIZE = new Dimension(1080 / 4 * 3 / 8, 1080 / 4 * 3 / 8);
    ...
    protected  ChessColor chessColor;  //暂时删去final关键字进行试验
    ...
    public abstract String getChessComponentName();
    ...
    /**
     * @param another 主要用于和另外一个棋子交换位置（交换两个棋子的chessboardPoint和Location）
     *                <br>
    ...
    public void swapLocation(ChessComponent another) {
        ...
        //交换两个棋子的chessboardPoint和Location
    ...
    protected void processMouseEvent(MouseEvent e) {
        ...
        if (e.getID() == MouseEvent.MOUSE_PRESSED) {
            ...
            this.passed = true;
            clickController.mouseEntered(this);
        } if (e.getID() == MouseEvent.MOUSE_EXITED){
            this.passed = false;
            clickController.mouseExisted(this);
        }
    }
    ...
    //5.10添加，在每个chessComponent的paintComponent方法中都添加了
    protected void mousePassedPaint(Graphics g){
        if (isPassed()){
    ...
    public void paintCanMoveTo(Graphics g){
        if (canMoveTo){
            ((Graphics2D) g).setColor(Color.RED);
            ((Graphics2D) g).setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, 0.3f));
            ((Graphics2D) g).fillOval(getWidth()/4, getHeight()/4, getWidth()/2 , getHeight()/2);
        }
    }
